...
import { Activity, Radio, Package, Truck, AlertTriangle } from 'lucide-react';
import Navbar from '../Navbar';
import Footer from '../Footer';

interface WebSocketEvent {
  id: string;
  type: 'shipment:status' | 'vehicle:location' | 'fleet:alert';
  timestamp: string;
...
const EVENT_TYPES = [
  ...
  { type: 'fleet:alert' as const, label: 'Fleet Alert', icon: AlertTriangle, color: 'purple' },
];
    ...
    <div className="min-h-screen bg-slate-50">
      ...
      <main className="max-w-6xl mx-auto px-4 py-10 space-y-6">
        <header>
          ...
          <h1 className="text-2xl font-bold text-gray-900 mt-2">Live Event Stream</h1>
          <p className="text-sm text-gray-600 mt-1">
            Visualize how ShipGen pushes fleet telemetry to browsers in milliseconds. This page uses rotating mock events so you
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <div className="text-sm text-gray-600">
                {isConnected ? 'Streaming demo updates every 4 seconds' : 'Live connection lost'}
              </div>
            </div>
          </div>
        </section>
            ...
            return (
              <article key={eventType.type} className="bg-white rounded-xl border border-gray-200 p-6">
                ...
                <p className="text-sm text-gray-600">
                  Pushes whenever {eventType.type.replace(':', ' ')} events fire inside the fleet control tower.
                </p>
              </article>
            );
          })}
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-6">
          ...
          <div className="space-y-4">
            ...
            <ArchitectureStep
              ...
              description="Shipment and fleet services publish domain events, which the broker fans out to live clients."
            />
            <ArchitectureStep
              number="03"
              title="React subscriber"
              description="The Live Operations UI listens to channels and re-renders instantly as fleet events arrive."
            />
          </div>
        </section>
...
const generateMockEvents = (count: number): WebSocketEvent[] => {
  const templates = [
    ...
    {
      type: 'vehicle:location' as const,
      data: () => ({
    ...
    {
      type: 'fleet:alert' as const,
      data: () => ({
        vehicleId: 'VH-' + Math.floor(Math.random() * 500),
        alert: ['Idle too long', 'Utilization drop', 'Maintenance due'][Math.floor(Math.random() * 3)],
...
const renderEventDescription = (event: WebSocketEvent) => {
  ...
  return `${event.data.alert} on ${event.data.vehicleId} (${event.data.severity})`;
};
